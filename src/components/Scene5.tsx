...
export class Scene5 implements Scene {
    ...
    description({ currentFrame }: { currentFrame: number; }): JSX.Element {
        ...
        const timelineSteps = [
            {
                title: "多段のブレーキ",
                description: "許容速度は先行する列車の位置に基づいて設定されますから、列車が近づくと、許容速度の変更に応じて自動的にブレーキをかけます。このように、ＡＴＣは列車間の安全な距離を維持して、衝突を防ぎます。"
            },
            {
                title: "多段のブレーキ",
                description: "この方式は駅での停車も使われます。日本初の新幹線列車モデルは、三つの異なる許容速度レベルを使いました。時速２１０ｋｍ、１６０ｋｍ、３０ｋｍです。列車が駅に近づくと、許容速度は段階的に下げられ、最終的に停車します。"
            },
            {
                title: "多段のブレーキ",
                description: "こう見ると、ＡＴＣシステムは列車の安全な運行を確保するために重要な役割をもっています。軌道回路と電流信号を利用して、列車の位置と速度を正確に把握し、必要に応じて自動的にブレーキをかけることで、乗客の安全を守っています。"
            },
        ];
